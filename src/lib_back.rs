...
fn init<'a>(
  ...
  let mut modelField = Field::new(
    ...
    |ctx| FieldFuture::new(async move{
        ...
        println!("serde got {:?}", serde_json::from_str::<PythonResponse>(&msg.as_str().unwrap()).unwrap());
        // let deserialized: PythonResponse = serde_json::from_str::<PythonResponse>(&msg.as_str().unwrap()).unwrap();
        // let deserialized: HashMap<String, serde_json::Value> = serde_json::from_str(&msg.as_str().unwrap()).unwrap();
